...
class Voiture extends React.Component {
    ...
    submitVoiture = (event) => {
        ...
        axios.post("http://localhost:8083/voitures", voiture)
            .then(response => {
                if (response.data != null) {
                    this.setState({ ...this.initialState, showSuccessToast: true, successMessage: "Voiture ajoutée avec succès." });
            ...
            <section className="relative">
                <div className="overflow-hidden rounded-3xl border border-white/60 bg-white/90 shadow-card backdrop-blur animate-fade-up">
                    ...
                    <div className="border-b border-slate-200/70 bg-white/80 px-6 py-5">
                        ...
                        <p className="mt-1 text-sm text-slate-500">Remplissez les champs ci-dessous</p>
                    </div>
